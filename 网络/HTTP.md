...
1. HTTP1.1有单个域名的并发请求上限（6个），这也就是为什么之前的CDN会分成好几个域名，以此来达到更大的并发请求数量。

## HTTP2.0的优势

1. 多路复用。全部http请求都用同一个tcp连接，这样减少了很多tcp开关时间，这解决了1.1时代单域名6个请求数限制的缺点；当然在弱网情况下，由于tcp的丢包重传机制，这也是一个劣势；

2. header压缩。其原理是在两端建立索引表，记录出现过的header，后面在传输过程中只需要传输键名就行了，不用传输键值。

## Cookie隔离是什么？

首先Cookie是有业务含义的，一般会用来做验证客户端身份；

这也意味着，其实我们的很多静态资源请求（不参与业务逻辑验证）是不需要Cookie的。

这时候，我们通常会把静态资源放在另外的CDN域名，这样主站只处理业务请求，给主站发的请求要带Cookie；给静态资源域名发的请求就不用带主站的cookie了。

少带cookie，可以让请求头体积更小，传输更快。
